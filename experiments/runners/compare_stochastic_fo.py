...
from experiments.common.provenance import json_value, save_profile
...
from experiments.common.experiment import (  # noqa: E402
    PAYOFF_MODELS,
    PAYOFF_POPULATIONS,
    RISKS,
    StreamingCsvWriter,
...
def run_instance(
    ...
    for method in args.methods:
        ...
        if getattr(args, "save_profiles", False) and args.csv is not None:
            config_id = hashlib.sha256(json.dumps(json_value(vars(args)), sort_keys=True).encode()).hexdigest()[:16]
            stem = args.csv.stem.removesuffix(".partial")
            path = args.csv.parent / "profiles" / f"{stem}__{risk}_K{K}_n{n}_seed{seed}__{method}_{config_id}.json"
            profile = (result.x, result.y) if result is not None else (None, None)
            save_profile(path, args=args, row=row, method=method, result=result, profile=profile, arrays=(A, B, p))
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument("--save-profiles", action="store_true", help="Save returned profiles and input provenance.")
