...
def run_mystic_seed(A_list, B_list, p, gamma, maxiter=2000, popsize=80, seed=13):
    ...
    sol = diffev2(cost, x0=x0, bounds=list(zip(lb, ub)),
                  maxiter=maxiter, popsize=popsize,
                  ftol=1e-10, gtol=50, itermon=mon,
...
def refine_with_pyomo(v_seed, A_list, B_list, p, gamma, solver="path"):
    ...
    for i in range(n1): m.x[i].value = float(x[i])
    for j in range(n2): m.y[j].value = float(y[j])
    for k in range(K):
    ...
    return {"x": xs, "y": ys, "alpha1": a1, "alpha2": a2, "lam1": l1, "z1": z1s, "lam2": l2, "z2": z2s, "pyomo_result": str(res)}
