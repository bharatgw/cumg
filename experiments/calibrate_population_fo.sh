...
# Usage: bash experiments/calibrate_population_fo.sh baseline
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR/.."
PHASE="${1:-baseline}"
PYTHON="${PYTHON:-python}"
if [[ "$PHASE" == cvar_* ]]; then
  RESULT_DIR="${RESULT_DIR:-experiments/results/calibration/population_fo/cvar_steps_v2}"
else
  RESULT_DIR="${RESULT_DIR:-experiments/results/calibration/population_fo/pilot_v1}"
fi
...
exec "$PYTHON" -u experiments/compare_stochastic_fo.py "${args[@]}" \
